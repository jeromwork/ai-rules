...
These **Meta Gates** are MANDATORY for every change. For each PR/patch, the agent MUST attach **evidence** per gate.
Reviewers MUST enforce this using `/.rules/meta/.rules/main/CODE_REVIEW.md`.
...
**Evidence (MUST):** problem statement, quality attributes list, ADR link (`/docs/adr/xxxx-title.md`).
